...
namespace std {
  template<> struct is_error_code_enum<flow::util::Flags::ErrorCode> : public std::true_type {};
} // namespace std

namespace fmt {
  template<>
  struct formatter<flow::util::Flags> {
    template <typename ParseContext>
    constexpr auto parse(ParseContext &ctx) { return ctx.begin(); }

    template <typename FormatContext>
    constexpr auto format(const flow::util::Flags& v, FormatContext &ctx) {
